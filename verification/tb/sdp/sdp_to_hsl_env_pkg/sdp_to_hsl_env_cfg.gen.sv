`ifndef SDP_TO_HSL_ENV_CFG__SV
`define SDP_TO_HSL_ENV_CFG__SV

class sdp_to_hsl_env_cfg extends uvm_object; 
  ...
  sdp_to_hsl_cfg m_sdp_to_hsl_cfg;
  ...
  sdp_to_hsl_block         ral;
  sdp_to_hsl_block ral_models[string];
  ...
  string ral_model_names[$] = {sdp_to_hsl_block::type_name};

  `uvm_object_param_utils_begin(sdp_to_hsl_env_cfg)
  ...
  virtual function void initialize(bit [31:0] csr_base_addr = '1);
    ...
    m_sdp_to_hsl_cfg = sdp_to_hsl_cfg::type_id::create("m_sdp_to_hsl_cfg");
  ...
  protected virtual function void pre_build_ral_settings(sdp_to_hsl_block ral);
  endfunction
  ...
  protected virtual function void post_build_ral_settings(sdp_to_hsl_block ral);
  endfunction
  ...
  virtual function void create_ral_models(bit [31:0] csr_base_addr = '1);
    ...
    foreach (ral_model_names[i]) begin
      ...
      sdp_to_hsl_block reg_blk = create_ral_by_name(ral_name);

      if (reg_blk.get_name() == sdp_to_hsl_block::type_name) $cast(ral, reg_blk);
    ...
    if (ral_model_names.size > 0) begin
      if (!ral_models.exists(sdp_to_hsl_block::type_name))
      `uvm_fatal(get_full_name(), "RAL ERROR")
    end
  endfunction

  virtual function sdp_to_hsl_block create_ral_by_name(string name);
    uvm_object        obj;
    uvm_factory       factory;
    sdp_to_hsl_block ral;
    ...
    if (!$cast(ral, obj)) begin
      `uvm_fatal(get_type_name(), $sformatf("cast failed - %0s is not a sdp_to_hsl_block", name))
    end
    return ral;
  endfunction:create_ral_by_name

endclass: sdp_to_hsl_env_cfg

`endif // SDP_TO_HSL_ENV_CFG__SV
