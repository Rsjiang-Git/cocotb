...
*/
`define CREATE_TEST_BEGIN(TEST=uvm_test) class run_each_seq_on_``TEST;   typedef TEST test_type;  local static bit tests_registered = register_tests();   local static function bit register_tests();     uvm_object_wrapper seqs[] = '{
`define ADD_SEQ(SEQ) SEQ::get_type(),
`define CREATE_TEST_END         uvm_sequence #(uvm_sequence_item)::get_type()};     foreach (seqs[i])       void'(test_builder #(test_type)::get(seqs[i]).register());   endfunction endclass
