...
class tfu_drv_callbacks extends uvm_callback;
   ...
   virtual task pre_tx( tfu_drv xactor, tfu_item tr);
   ...
   virtual task post_tx( tfu_drv xactor, tfu_item tr);
     // ToDo: Add relevant code
...
task tfu_drv::tx_driver();
 forever begin
      ...
      endcase
	   `uvm_do_callbacks(tfu_drv, tfu_drv_callbacks, pre_tx(this, tr))
      send(tr); 
      seq_item_port.item_done();
      `uvm_info("TX_DRIVER", "Completed transaction...",UVM_LOW)
      `uvm_info("TX_DRIVER", tr.sprint(),UVM_HIGH)
      `uvm_do_callbacks(tfu_drv, tfu_drv_callbacks, post_tx(this, tr))
