...
class tfu_env_cfg extends uvm_object; 
  ...
  string ral_model_names[$] = {tfu_block::type_name};
  ...
  protected virtual function void pre_build_ral_settings(tfu_block ral);
  endfunction
  ...
  protected virtual function void post_build_ral_settings(tfu_block ral);
  endfunction
  ...
  virtual function void create_ral_models(bit [31:0] csr_base_addr = '1);
    ...
    foreach (ral_model_names[i]) begin
      ...
      tfu_block reg_blk = create_ral_by_name(ral_name);

      if (reg_blk.get_name() == tfu_block::type_name) $cast(ral, reg_blk);
    ...
    if (ral_model_names.size > 0) begin
      if (!ral_models.exists(tfu_block::type_name))
      `uvm_fatal(get_full_name(), "RAL ERROR")
    end
  endfunction

  virtual function tfu_block create_ral_by_name(string name);
    uvm_object        obj;
    uvm_factory       factory;
    tfu_block ral;
    ...
    if (!$cast(ral, obj)) begin
      `uvm_fatal(get_type_name(), $sformatf("cast failed - %0s is not a tfu_block", name))
    end
    return ral;
