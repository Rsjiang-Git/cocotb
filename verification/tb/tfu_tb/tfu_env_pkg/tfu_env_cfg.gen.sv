`ifndef TFU_ENV_CFG__SV
`define TFU_ENV_CFG__SV

class tfu_env_cfg extends uvm_object; 
  ...
  tfu_cfg m_tfu_cfg;
  m_svt_system_cfg svt_system_cfg;
  ...
  empty_reg_block         ral;
  empty_reg_block ral_models[string];
  ...
  string ral_model_names[$] = {empty_reg_block::type_name};

  `uvm_object_param_utils_begin(tfu_env_cfg)
  ...
  virtual function void initialize(bit [31:0] csr_base_addr = '1);
    ...
    m_tfu_cfg = tfu_cfg::type_id::create("m_tfu_cfg");
    svt_system_cfg = m_svt_system_cfg::type_id::create("svt_system_cfg");
  ...
  protected virtual function void pre_build_ral_settings(empty_reg_block ral);
  endfunction
  ...
  protected virtual function void post_build_ral_settings(empty_reg_block ral);
  endfunction
  ...
  virtual function void create_ral_models(bit [31:0] csr_base_addr = '1);
    ...
    foreach (ral_model_names[i]) begin
      ...
      empty_reg_block reg_blk = create_ral_by_name(ral_name);

      if (reg_blk.get_name() == empty_reg_block::type_name) $cast(ral, reg_blk);
    ...
    if (ral_model_names.size > 0) begin
      if (!ral_models.exists(empty_reg_block::type_name))
      `uvm_fatal(get_full_name(), "RAL ERROR")
    end
  endfunction

  virtual function empty_reg_block create_ral_by_name(string name);
    uvm_object        obj;
    uvm_factory       factory;
    empty_reg_block ral;
    ...
    if (!$cast(ral, obj)) begin
      `uvm_fatal(get_type_name(), $sformatf("cast failed - %0s is not a empty_reg_block", name))
    end
    return ral;
  endfunction:create_ral_by_name

endclass: tfu_env_cfg

`endif // TFU_ENV_CFG__SV
